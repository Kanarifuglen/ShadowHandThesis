...
def downsample_indices(total_frames, target_frames=300):
    """
    Generate indices to downsample a sequence uniformly to `target_frames` length.
    If total_frames <= target_frames, returns full range (no downsampling).
    """
    if total_frames <= target_frames:
        return np.arange(total_frames)
    else:
        return np.linspace(0, total_frames - 1, target_frames).astype(int)
